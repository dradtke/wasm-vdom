...
func TestNewTree(t *testing.T) {
	tree, err := vdom.NewTree(`<div><span>Hello</span><span>World</span></div>`)
	if err != nil {
		t.Fatal(err)
	}
	...
	if tree.Path != nil {
		t.Fatal("expected root to have nil path")
	}
	...
	if !reflect.DeepEqual(helloTree.Path, []int{0}) {
		t.Fatalf("expected hello to have path of [0] (was: %v)", helloTree.Path)
	}
	...
	if !reflect.DeepEqual(worldTree.Path, []int{1}) {
		t.Fatalf("expected hello to have path of [1] (was: %v)", worldTree.Path)
	}
